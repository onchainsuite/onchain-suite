...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!session || !session.session || !session.session.activeOrganizationId) {
      return NextResponse.json({ isActive: false, status: "unknown" }, { status: 401 });
    }
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "https://onchain-backend-dvxw.onrender.com"}/api/v1/organization`, {
      headers: {
    ...
    return NextResponse.json({ isActive: true, status: "error" }, { status: 500 });
  }
}
